 ...
 */
function resolveMonacoPath(filePath, sseEditorPath) {
    if (sseEditorPath) {
        return require.resolve(path.join(sseEditorPath, 'esm', filePath));
    }
    try {
 ...
 */
function getWorkerFilename(filename, entry, sseEditorPath) {
    return loaderUtils.interpolateName({ resourcePath: entry }, filename, {
        content: fs.readFileSync(resolveMonacoPath(entry, sseEditorPath))
    });
}
function getEditorMetadata(sseEditorPath) {
    const metadataPath = resolveMonacoPath('metadata.js', sseEditorPath);
...
class MonacoEditorWebpackPlugin {
    constructor(options = {}) {
        const sseEditorPath = options.sseEditorPath;
        const metadata = getEditorMetadata(sseEditorPath);
    ...
    apply(compiler) {
        const { languages, features, filename, sseEditorPath, publicPath, globalAPI } = this.options;
        ...
        const rules = createLoaderRules(languages, features, workers, filename, sseEditorPath, publicPath, compilationPublicPath, globalAPI);
        const plugins = createPlugins(compiler, workers, filename, sseEditorPath);
...
function createLoaderRules(languages, features, workers, filename, sseEditorPath, pluginPublicPath, compilationPublicPath, globalAPI) {
    ...
    const workerPaths = fromPairs(workers.map(({ label, entry }) => [label, getWorkerFilename(filename, entry, sseEditorPath)]));
    ...
    const options = {
        globals,
        pre: featurePaths.map((importPath) => resolveMonacoPath(importPath, sseEditorPath)),
        post: languagePaths.map((importPath) => resolveMonacoPath(importPath, sseEditorPath))
    };
    return [
        {
...
function createPlugins(compiler, workers, filename, sseEditorPath) {
    var _a;
    ...
    return [].concat(workers.map(({ id, entry }) => new AddWorkerEntryPointPlugin_1.AddWorkerEntryPointPlugin({
        id,
        entry: resolveMonacoPath(entry, sseEditorPath),
        filename: getWorkerFilename(filename, entry, sseEditorPath),
