...
} from '@sseworld/code-editor/esm/metadata';
 ...
 */
function resolveMonacoPath(filePath: string, sseEditorPath: string | undefined): string {
	if (sseEditorPath) {
...
function getWorkerFilename(
	filename: string,
	entry: string,
	sseEditorPath: string | undefined
): string {
...
function getEditorMetadata(sseEditorPath: string | undefined): EditorMetadata {
...
function createLoaderRules(
	...
	const workerPaths = fromPairs(
		workers.map(({ label, entry }) => [label, getWorkerFilename(filename, entry, sseEditorPath)])
	);
