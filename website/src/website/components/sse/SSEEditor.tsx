...
import { getLoadedSSE } from "../../../sse-loader";
import { withLoadedMonaco } from "./SSELoader";
...
export class ControlledMonacoEditor extends React.Component<{
	...
	private readonly model = getLoadedSSE().editor.createModel(
		this.props.value,
		this.props.language
	);
	...
	componentDidUpdate(lastProps: this["props"]) {
		...
		if (this.model.getLanguageId() !== this.props.language) {
			getLoadedSSE().editor.setModelLanguage(
				this.model,
...
export class ControlledMonacoDiffEditor extends React.Component<{
	...
	private readonly originalModel = getLoadedSSE().editor.createModel(
		this.props.originalValue,
		this.props.language
	);
	private readonly modifiedModel = getLoadedSSE().editor.createModel(
...
> {
	...
	componentDidMount() {
		...
		this.editor = getLoadedSSE().editor.create(div, {
	...
	componentDidUpdate(oldProps: this["props"]) {
		...
		if (oldProps.theme !== this.props.theme && this.props.theme) {
			getLoadedSSE().editor.setTheme(this.props.theme);
		}
...
> {
	...
	componentDidMount() {
		...
		this.resizeObserver.observe(div);
		this.editor = getLoadedSSE().editor.createDiffEditor(div, {
			scrollBeyondLastLine: false,
			minimap: { enabled: false },
			automaticLayout: false,
			theme: this.props.theme,
