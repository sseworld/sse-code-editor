...
var moduleExports = (() => {
  ...
  var WorkerManager = class {
    ...
    _getClient() {
      if (!this._client) {
        this._client = (async () => {
          this._worker = sse_editor_core_exports.editor.createWebWorker({
          ...
          if (this._defaults.getEagerModelSync()) {
            return await this._worker.withSyncedResources(
              sse_editor_core_exports.editor.getModels().filter((model) => model.getLanguageId() === this._modeId).map((model) => model.uri)
            );
          }
  ...
  var import_sse = __require("./sse.contribution");
  ...
  var LibFiles = class {
    ...
    getOrCreateModel(fileName) {
      const uri = sse_editor_core_exports.Uri.parse(fileName);
      const model = sse_editor_core_exports.editor.getModel(uri);
      if (model) {
        return model;
      }
      if (this.isLibFile(uri) && this._hasFetchedLibFiles) {
        return sse_editor_core_exports.editor.createModel(this._libFiles[uri.path.slice(1)], "typescript", uri);
      }
      const matchedLibFile = import_sse.typescriptDefaults.getExtraLibs()[fileName];
      if (matchedLibFile) {
        return sse_editor_core_exports.editor.createModel(matchedLibFile.content, "typescript", uri);
      }
      return null;
    }
  ...
  var DiagnosticsAdapter = class extends Adapter {
    constructor(_libFiles, _defaults, _selector, worker) {
        ...
        const visibleSubscription = model.onDidChangeAttached(() => {
          ...
          if (onlyVisible) {
            if (model.isAttachedToEditor()) {
              ...
              sse_editor_core_exports.editor.setModelMarkers(model, this._selector, []);
            }
          }
        });
      ...
      this._disposables.push(
        sse_editor_core_exports.editor.onDidCreateModel((model) => onModelAdd(model))
      );
      this._disposables.push(sse_editor_core_exports.editor.onWillDisposeModel(onModelRemoved));
      this._disposables.push(
        sse_editor_core_exports.editor.onDidChangeModelLanguage((event) => {
      ...
      this._disposables.push({
        dispose() {
          for (const model of sse_editor_core_exports.editor.getModels()) {
      ...
      sse_editor_core_exports.editor.getModels().forEach((model) => onModelAdd(model));
    }
    dispose() {
    ...
    async _doValidate(model) {
      ...
      const relatedUris = diagnostics.map((d) => d.relatedInformation || []).reduce((p, c) => c.concat(p), []).map(
        (relatedInformation) => relatedInformation.file ? sse_editor_core_exports.Uri.parse(relatedInformation.file.fileName) : null
      );
      await this._libFiles.fetchLibFilesIfNecessary(relatedUris);
      if (model.isDisposed()) {
        return;
      }
      sse_editor_core_exports.editor.setModelMarkers(
        model,
        this._selector,
    ...
    _convertDiagnostics(model, diag) {
      ...
      if (diag.reportsUnnecessary) {
        tags.push(sse_editor_core_exports.MarkerTag.Unnecessary);
      }
      if (diag.reportsDeprecated) {
        tags.push(sse_editor_core_exports.MarkerTag.Deprecated);
      }
      return {
    ...
    _tsDiagnosticCategoryToMarkerSeverity(category) {
      switch (category) {
        case 1 /* Error */:
          return sse_editor_core_exports.MarkerSeverity.Error;
        case 3 /* Message */:
          return sse_editor_core_exports.MarkerSeverity.Info;
        case 0 /* Warning */:
          return sse_editor_core_exports.MarkerSeverity.Warning;
        case 2 /* Suggestion */:
          return sse_editor_core_exports.MarkerSeverity.Hint;
      }
      return sse_editor_core_exports.MarkerSeverity.Info;
    }
  };
  var SuggestAdapter = class _SuggestAdapter extends Adapter {
    ...
    async provideCompletionItems(model, position, _context, token) {
      ...
      const wordRange = new sse_editor_core_exports.Range(
        position.lineNumber,
      ...
      const suggestions = info.entries.map((entry) => {
        ...
        if (entry.replacementSpan) {
          ...
          range = new sse_editor_core_exports.Range(p1.lineNumber, p1.column, p2.lineNumber, p2.column);
        }
        const tags = [];
        if (entry.kindModifiers !== void 0 && entry.kindModifiers.indexOf("deprecated") !== -1) {
          tags.push(sse_editor_core_exports.languages.CompletionItemTag.Deprecated);
        }
        return {
          uri: resource,
          position,
    ...
    static convertKind(kind) {
      switch (kind) {
        ...
        case Kind.keyword:
          return sse_editor_core_exports.languages.CompletionItemKind.Keyword;
        case Kind.variable:
        case Kind.localVariable:
          return sse_editor_core_exports.languages.CompletionItemKind.Variable;
        case Kind.memberVariable:
        case Kind.memberGetAccessor:
        case Kind.memberSetAccessor:
          return sse_editor_core_exports.languages.CompletionItemKind.Field;
        case Kind.function:
        case Kind.memberFunction:
        case Kind.constructSignature:
        case Kind.callSignature:
        case Kind.indexSignature:
          return sse_editor_core_exports.languages.CompletionItemKind.Function;
        case Kind.enum:
          return sse_editor_core_exports.languages.CompletionItemKind.Enum;
        case Kind.module:
          return sse_editor_core_exports.languages.CompletionItemKind.Module;
        case Kind.class:
          return sse_editor_core_exports.languages.CompletionItemKind.Class;
        case Kind.interface:
          return sse_editor_core_exports.languages.CompletionItemKind.Interface;
        case Kind.warning:
          return sse_editor_core_exports.languages.CompletionItemKind.File;
      }
      return sse_editor_core_exports.languages.CompletionItemKind.Property;
    }
...
${tagToString(tag)}`;
  ...
  var SignatureHelpAdapter = class _SignatureHelpAdapter extends Adapter {
    ...
    static _toSignatureHelpTriggerReason(context) {
      switch (context.triggerKind) {
        case sse_editor_core_exports.languages.SignatureHelpTriggerKind.TriggerCharacter:
        ...
        case sse_editor_core_exports.languages.SignatureHelpTriggerKind.ContentChange:
          return context.isRetrigger ? { kind: "retrigger" } : { kind: "invoked" };
        case sse_editor_core_exports.languages.SignatureHelpTriggerKind.Invoke:
        default:
          return { kind: "invoked" };
      }
    }
      ...
      return entries.flatMap((entry) => {
        return entry.highlightSpans.map((highlightSpans) => {
          return {
            ...
            kind: highlightSpans.kind === "writtenReference" ? sse_editor_core_exports.languages.DocumentHighlightKind.Write : sse_editor_core_exports.languages.DocumentHighlightKind.Text
          };
        });
      });
    }
  ...
  var DefinitionAdapter = class extends Adapter {
    ...
    async provideDefinition(model, position, token) {
      ...
      await this._libFiles.fetchLibFilesIfNecessary(
        entries.map((entry) => sse_editor_core_exports.Uri.parse(entry.fileName))
      );
      if (model.isDisposed()) {
        return;
      }
    ...
    async provideDocumentSymbols(model, token) {
      ...
      const convert = (item, containerLabel) => {
        const result2 = {
          name: item.text,
          detail: "",
          kind: outlineTypeTable[item.kind] || sse_editor_core_exports.languages.SymbolKind.Variable,
  ...
  outlineTypeTable[Kind.module] = sse_editor_core_exports.languages.SymbolKind.Module;
  outlineTypeTable[Kind.class] = sse_editor_core_exports.languages.SymbolKind.Class;
  outlineTypeTable[Kind.enum] = sse_editor_core_exports.languages.SymbolKind.Enum;
  outlineTypeTable[Kind.interface] = sse_editor_core_exports.languages.SymbolKind.Interface;
  outlineTypeTable[Kind.memberFunction] = sse_editor_core_exports.languages.SymbolKind.Method;
  outlineTypeTable[Kind.memberVariable] = sse_editor_core_exports.languages.SymbolKind.Property;
  outlineTypeTable[Kind.memberGetAccessor] = sse_editor_core_exports.languages.SymbolKind.Property;
  outlineTypeTable[Kind.memberSetAccessor] = sse_editor_core_exports.languages.SymbolKind.Property;
  outlineTypeTable[Kind.variable] = sse_editor_core_exports.languages.SymbolKind.Variable;
  outlineTypeTable[Kind.const] = sse_editor_core_exports.languages.SymbolKind.Variable;
  outlineTypeTable[Kind.localVariable] = sse_editor_core_exports.languages.SymbolKind.Variable;
  outlineTypeTable[Kind.variable] = sse_editor_core_exports.languages.SymbolKind.Variable;
  outlineTypeTable[Kind.function] = sse_editor_core_exports.languages.SymbolKind.Function;
  outlineTypeTable[Kind.localFunction] = sse_editor_core_exports.languages.SymbolKind.Function;
  ...
  var InlayHintsAdapter = class extends Adapter {
    ...
    _convertHintKind(kind) {
      switch (kind) {
        case "Parameter":
          return sse_editor_core_exports.languages.InlayHintKind.Parameter;
        case "Type":
          return sse_editor_core_exports.languages.InlayHintKind.Type;
        default:
          return sse_editor_core_exports.languages.InlayHintKind.Type;
      }
    }
  ...
  function setupMode(defaults, modeId) {
    ...
    function registerProviders() {
      ...
      if (modeConfiguration.completionItems) {
        providers.push(
          sse_editor_core_exports.languages.registerCompletionItemProvider(
            modeId,
      ...
      if (modeConfiguration.signatureHelp) {
        providers.push(
          sse_editor_core_exports.languages.registerSignatureHelpProvider(
            modeId,
      ...
      if (modeConfiguration.hovers) {
        providers.push(
          sse_editor_core_exports.languages.registerHoverProvider(modeId, new QuickInfoAdapter(worker))
        );
      }
      if (modeConfiguration.documentHighlights) {
        providers.push(
          sse_editor_core_exports.languages.registerDocumentHighlightProvider(
            modeId,
      ...
      if (modeConfiguration.definitions) {
        providers.push(
          sse_editor_core_exports.languages.registerDefinitionProvider(
            modeId,
      ...
      if (modeConfiguration.references) {
        providers.push(
          sse_editor_core_exports.languages.registerReferenceProvider(
            modeId,
      ...
      if (modeConfiguration.documentSymbols) {
        providers.push(
          sse_editor_core_exports.languages.registerDocumentSymbolProvider(
            modeId,
      ...
      if (modeConfiguration.rename) {
        providers.push(
          sse_editor_core_exports.languages.registerRenameProvider(
            modeId,
      ...
      if (modeConfiguration.documentRangeFormattingEdits) {
        providers.push(
          sse_editor_core_exports.languages.registerDocumentRangeFormattingEditProvider(
            modeId,
      ...
      if (modeConfiguration.onTypeFormattingEdits) {
        providers.push(
          sse_editor_core_exports.languages.registerOnTypeFormattingEditProvider(
            modeId,
      ...
      if (modeConfiguration.codeActions) {
        providers.push(
          sse_editor_core_exports.languages.registerCodeActionProvider(modeId, new CodeActionAdaptor(worker))
        );
      }
      if (modeConfiguration.inlayHints) {
        providers.push(
          sse_editor_core_exports.languages.registerInlayHintsProvider(modeId, new InlayHintsAdapter(worker))
        );
      }
