...
var moduleExports = (() => {
  ...
  var conf = {
    ...
    onEnterRules: [
      {
        beforeText: /^\s*- .+/,
        action: { indentAction: sse_editor_core_exports.languages.IndentAction.None, appendText: "- " }
      },
      {
        beforeText: /^\s*\+ .+/,
        action: { indentAction: sse_editor_core_exports.languages.IndentAction.None, appendText: "+ " }
      ...
      {
        beforeText: /<\w+/,
        action: { indentAction: sse_editor_core_exports.languages.IndentAction.Indent }
      },
      {
        beforeText: /\s+>\s*$/,
        action: { indentAction: sse_editor_core_exports.languages.IndentAction.Indent }
      ...
      {
        beforeText: /<\/\w+>/,
        action: { indentAction: sse_editor_core_exports.languages.IndentAction.Outdent }
      },
      ...Array.from({ length: 100 }, (_, index) => ({
        beforeText: new RegExp(`^${index}\\. .+`),
        action: { indentAction: sse_editor_core_exports.languages.IndentAction.None, appendText: `${index + 1}. ` }
      }))
    ]
  };
  var language = {
