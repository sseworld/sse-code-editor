...
var moduleExports = (() => {
  ...
  var DiagnosticsAdapter = class {
    constructor(_languageId, _worker, configChangeEvent) {
      ...
      const onModelRemoved = (model) => {
        sse_editor_core_exports.editor.setModelMarkers(model, this._languageId, []);
      ...
      this._disposables.push(sse_editor_core_exports.editor.onDidCreateModel(onModelAdd));
      ...
      this._disposables.push(
        configChangeEvent((_) => {
          sse_editor_core_exports.editor.getModels().forEach((model) => {
      ...
      this._disposables.push({
        dispose: () => {
          sse_editor_core_exports.editor.getModels().forEach(onModelRemoved);
          for (let key in this._listener) {
            this._listener[key].dispose();
          }
        }
      });
      sse_editor_core_exports.editor.getModels().forEach(onModelAdd);
    }
    dispose() {
    ...
    _doValidate(resource, languageId) {
      this._worker(resource).then((worker2) => {
        ...
        let model = sse_editor_core_exports.editor.getModel(resource);
        if (model && model.getLanguageId() === languageId) {
          sse_editor_core_exports.editor.setModelMarkers(model, languageId, markers);
        }
      }).then(void 0, (err) => {
        console.error(err);
      });
      ...
      return this._worker(resource).then((worker2) => {
        ...
        const items = info.items.map((entry) => {
          ...
          if (entry.insertTextFormat === InsertTextFormat.Snippet) {
            item.insertTextRules = sse_editor_core_exports.languages.CompletionItemInsertTextRule.InsertAsSnippet;
          }
          return item;
        });
        return {
  ...
  function toRange(range) {
    ...
    return new sse_editor_core_exports.Range(
      range.start.line + 1,
  ...
  function toCompletionItemKind(kind) {
    const mItemKind = sse_editor_core_exports.languages.CompletionItemKind;
    switch (kind) {
  ...
  function toDocumentHighlightKind(kind) {
    switch (kind) {
      case DocumentHighlightKind.Read:
        return sse_editor_core_exports.languages.DocumentHighlightKind.Read;
      case DocumentHighlightKind.Write:
        return sse_editor_core_exports.languages.DocumentHighlightKind.Write;
      case DocumentHighlightKind.Text:
        return sse_editor_core_exports.languages.DocumentHighlightKind.Text;
    }
    return sse_editor_core_exports.languages.DocumentHighlightKind.Text;
  }
  ...
  function toLocation(location) {
    return {
      uri: sse_editor_core_exports.Uri.parse(location.uri),
  ...
  function toWorkspaceEdit(edit) {
    ...
    for (let uri in edit.changes) {
      const _uri = sse_editor_core_exports.Uri.parse(uri);
  ...
  function toSymbolKind(kind) {
    let mKind = sse_editor_core_exports.languages.SymbolKind;
    switch (kind) {
  ...
  function toFoldingRangeKind(kind) {
    switch (kind) {
      case FoldingRangeKind.Comment:
        return sse_editor_core_exports.languages.FoldingRangeKind.Comment;
      case FoldingRangeKind.Imports:
        return sse_editor_core_exports.languages.FoldingRangeKind.Imports;
      case FoldingRangeKind.Region:
        return sse_editor_core_exports.languages.FoldingRangeKind.Region;
    }
    return void 0;
  }
  ...
  var JSONDiagnosticsAdapter = class extends DiagnosticsAdapter {
    constructor(languageId, worker2, defaults) {
      ...
      this._disposables.push(
        sse_editor_core_exports.editor.onWillDisposeModel((model) => {
  ...
  function setupMode(defaults) {
    ...
    function registerProviders() {
      ...
      if (modeConfiguration2.documentFormattingEdits) {
        providers.push(
          sse_editor_core_exports.languages.registerDocumentFormattingEditProvider(
            languageId,
      ...
      if (modeConfiguration2.hovers) {
        providers.push(
          sse_editor_core_exports.languages.registerHoverProvider(languageId, new HoverAdapter(worker))
        );
      }
      ...
      if (modeConfiguration2.tokens) {
        providers.push(sse_editor_core_exports.languages.setTokensProvider(languageId, createTokenizationSupport(true)));
      }
      if (modeConfiguration2.colors) {
        providers.push(
          sse_editor_core_exports.languages.registerColorProvider(
            languageId,
      ...
      if (modeConfiguration2.foldingRanges) {
        providers.push(
          sse_editor_core_exports.languages.registerFoldingRangeProvider(
            languageId,
      ...
      if (modeConfiguration2.selectionRanges) {
        providers.push(
          sse_editor_core_exports.languages.registerSelectionRangeProvider(
            languageId,
    ...
    registerProviders();
    disposables.push(sse_editor_core_exports.languages.setLanguageConfiguration(defaults.languageId, richEditConfiguration));
