 ...
 *--------------------------------------------------------------------------------------------*/
define(["require", "exports", "vscode-languageserver-types", "../../fillers/sse-editor-core"], function (require, exports, lsTypes, sse_editor_core_1) {
    "use strict";
    ...
    var DiagnosticsAdapter = /** @class */ (function () {
        function DiagnosticsAdapter(_languageId, _worker, configChangeEvent) {
            ...
            var onModelRemoved = function (model) {
                sse_editor_core_1.editor.setModelMarkers(model, _this._languageId, []);
            ...
            this._disposables.push(sse_editor_core_1.editor.onDidCreateModel(onModelAdd));
            this._disposables.push(sse_editor_core_1.editor.onWillDisposeModel(onModelRemoved));
            this._disposables.push(sse_editor_core_1.editor.onDidChangeModelLanguage(function (event) {
                onModelRemoved(event.model);
                onModelAdd(event.model);
            }));
            this._disposables.push(configChangeEvent(function (_) {
                sse_editor_core_1.editor.getModels().forEach(function (model) {
            ...
            this._disposables.push({
                dispose: function () {
                    sse_editor_core_1.editor.getModels().forEach(onModelRemoved);
                    for (var key in _this._listener) {
                        _this._listener[key].dispose();
                    }
                }
            });
            sse_editor_core_1.editor.getModels().forEach(onModelAdd);
        }
        ...
        DiagnosticsAdapter.prototype._doValidate = function (resource, languageId) {
            this._worker(resource)
                ...
                var model = sse_editor_core_1.editor.getModel(resource);
                if (model && model.getLanguageId() === languageId) {
                    sse_editor_core_1.editor.setModelMarkers(model, languageId, markers);
                }
            })
    ...
    var CompletionAdapter = /** @class */ (function () {
        ...
        CompletionAdapter.prototype.provideCompletionItems = function (model, position, context, token) {
            ...
            return this._worker(resource)
                ...
                var wordRange = new sse_editor_core_1.Range(position.lineNumber, wordInfo.startColumn, position.lineNumber, wordInfo.endColumn);
                var items = info.items.map(function (entry) {
                    ...
                    if (entry.insertTextFormat === lsTypes.InsertTextFormat.Snippet) {
                        item.insertTextRules = sse_editor_core_1.languages.CompletionItemInsertTextRule.InsertAsSnippet;
                    }
                    return item;
                });
                return {
    ...
    function toRange(range) {
        ...
        return new sse_editor_core_1.Range(range.start.line + 1, range.start.character + 1, range.end.line + 1, range.end.character + 1);
    }
    ...
    function toCompletionItemKind(kind) {
        var mItemKind = sse_editor_core_1.languages.CompletionItemKind;
        switch (kind) {
    ...
    function toDocumentHighlightKind(kind) {
        switch (kind) {
            case lsTypes.DocumentHighlightKind.Read:
                return sse_editor_core_1.languages.DocumentHighlightKind.Read;
            case lsTypes.DocumentHighlightKind.Write:
                return sse_editor_core_1.languages.DocumentHighlightKind.Write;
            case lsTypes.DocumentHighlightKind.Text:
                return sse_editor_core_1.languages.DocumentHighlightKind.Text;
        }
        return sse_editor_core_1.languages.DocumentHighlightKind.Text;
    }
    ...
    function toLocation(location) {
        return {
            uri: sse_editor_core_1.Uri.parse(location.uri),
    ...
    function toWorkspaceEdit(edit) {
        ...
        for (var uri in edit.changes) {
            var _uri = sse_editor_core_1.Uri.parse(uri);
    ...
    function toSymbolKind(kind) {
        var mKind = sse_editor_core_1.languages.SymbolKind;
        switch (kind) {
    ...
    function toFoldingRangeKind(kind) {
        switch (kind) {
            case lsTypes.FoldingRangeKind.Comment:
                return sse_editor_core_1.languages.FoldingRangeKind.Comment;
            case lsTypes.FoldingRangeKind.Imports:
                return sse_editor_core_1.languages.FoldingRangeKind.Imports;
            case lsTypes.FoldingRangeKind.Region:
                return sse_editor_core_1.languages.FoldingRangeKind.Region;
        }
        return void 0;
    }
