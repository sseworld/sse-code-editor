...
import { REPO_ROOT, readFiles, writeFiles, IFile } from './utils';
import { removeDir } from './fs';
import ts = require('typescript');
import { generateMetadata } from './releaseMetadata';

removeDir(`out/sse-editor`);
...
(() => {
	...
	writeFiles([packageJSON], `out/sse-editor`);
})();
...
(() => {
	...
	writeFiles(otherFiles, `out/sse-editor`);
})();
...
function AMD_releaseOne(type: 'dev' | 'min') {
	...
	writeFiles(coreFiles, `out/sse-editor/${type}`);

	const pluginFiles = readFiles(`out/languages/bundled/amd-${type}/**/*`, {
		base: `out/languages/bundled/amd-${type}`,
		ignore: ['**/sse.contribution.js']
	});
	writeFiles(pluginFiles, `out/sse-editor/${type}`);
}
...
function AMD_addPluginContribs(type: 'dev' | 'min', files: IFile[]) {
	for (const file of files) {
		...
		const pluginFiles = readFiles(`out/languages/bundled/amd-${type}/**/sse.contribution.js`, {
			base: `out/languages/bundled/amd-${type}`
		});
...
function ESM_release() {
	...
	writeFiles(coreFiles, `out/sse-editor/esm`);
...
function ESM_releasePlugins() {
	...
	writeFiles(files, `out/sse-editor/esm`);
}
...
function ESM_addPluginContribs(files: IFile[]) {
	...
	const mainFileImports = readFiles(`out/languages/bundled/esm/**/sse.contribution.js`, {
...
function releaseDTS() {
	...
	writeFiles([monacodts, editorapidts], `out/sse-editor`);
...
function toExternalDTS(contents: string): string {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if ('declare namespace sse {' === line) {
			lines[i] = '';
			killNextCloseCurlyBrace = true;
			continue;
		}

		if (line.indexOf('declare namespace sse.') === 0) {
			lines[i] = line.replace('declare namespace sse.', 'export namespace ');
		}
