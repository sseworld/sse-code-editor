...
function getBasicLanguages(): Promise<{ label: string; entry: string }[]> {
	return new Promise((resolve, reject) => {
		glob(
			'./out/sse-editor/esm/vs/basic-languages/*/*.contribution.js',
			...
			(err, files) => {
				...
				resolve(
					files.map((file) => {
						const entry = file.substring('./out/sse-editor/esm/'.length).replace(/\.js$/, '');
...
function readAdvancedLanguages(): Promise<string[]> {
	return new Promise((resolve, reject) => {
		glob(
			'./out/sse-editor/esm/vs/language/*/sse.contribution.js',
			{ cwd: path.dirname(__dirname) },
			(err, files) => {
				if (err) {
					reject(err);
				...
				resolve(
					files
						.map((file) => file.substring('./out/sse-editor/esm/vs/language/'.length))
						.map((file) => file.substring(0, file.length - '/sse.contribution.js'.length))
				);
			}
		);
	});
...
> {
	return readAdvancedLanguages().then((languages) => {
		...
		for (const lang of languages) {
			...
			const entry = `vs/language/${lang}/sse.contribution`;
	...
	function checkFileExists(moduleName) {
		const filePath = path.join(REPO_ROOT, 'out/sse-editor/esm', `${moduleName}.js`);
...
`;
			const dtsDestination = path.join(REPO_ROOT, 'out/sse-editor/esm/metadata.d.ts');
...
`;
			const jsDestination = path.join(REPO_ROOT, 'out/sse-editor/esm/metadata.js');
			...
			for (const feature of [...features, ...languages]) {
				...
				for (const entry of entries) {
					const dtsDestination = path.join(REPO_ROOT, 'out/sse-editor/esm', entry) + '.d.ts';
					ensureDir(path.dirname(dtsDestination));
...
function getFeatures(): { label: string; entry: string | string[] }[] {
	...
	const files =
		fs
			.readFileSync(path.join(REPO_ROOT, 'out/sse-editor/esm/vs/editor/edcore.main.js'))
			.toString() +
		fs
			.readFileSync(path.join(REPO_ROOT, 'out/sse-editor/esm/vs/editor/editor.all.js'))
			.toString();
	files.split(/\r\n|\n/).forEach((line) => {
